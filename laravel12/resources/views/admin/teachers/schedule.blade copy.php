                ...
                <tbody>
                    @forelse($timeSlots as $timeSlot)
                        <tr>
                            ...
                            @foreach($dayLabels as $dayNumber => $dayName)
                                @php
                                    $cell = $matrix[$timeSlot][$dayNumber];
                                @endphp
                                ...
                                @if($cell['type'] === 'start')
                                    <td rowspan="{{ $cell['rowspan'] }}" style="vertical-align:top;">
                                        @foreach($cell['items'] as $item)
                                            <div style="border:1px solid #e5e7eb; border-radius:10px; padding:10px; background:#f9fafb; min-height:100%;">
